...
export default function Sidebar() {
 ...
 const menuItems = [
  ...
  { href: '/Users', icon: 'ri-user-line', label: 'Users' },
      ...
      <div className="app-brand demo">
        <Link href="/" className="app-brand-link">
         <span className="app-brand-logo demo mt-2">
          ...
          <span className="app-brand-text demo menu-text fw-semibold ms-2 ">Flow 108</span>
        </Link>
        ...
        {menuItems.map((item) =>
          item.submenu ? (
            <li
              key={item.label}
              className={`menu-item ${
                item.submenu.some((sub) => pathname.startsWith(sub.href)) ? 'active open' : ''
              }`}
            >
