...
export default function BannerAssignmentModal({ isOpen, onClose, bannerId, bannerName, onAssignmentSuccess }) {
  ...
  const handleAssignBanner = async () => {
    ...
    try {
      const result = await assignBanner(bannerId, userIdsToAssign, assignToAll);
      if (result.status) {
        ...
        throw new Error(result.message || "Failed to assign banner");
      }
    } catch (error) {
      setAssignError(error.message || "Failed to assign banner");
    } finally {
      ...
      <div className="modal-dialog modal-lg" role="document">
        <div className="modal-content">
          <div className="modal-header">
            <h5 className="modal-title">Assign Banner: {bannerName}</h5>
            <button type="button" className="btn-close" onClick={onClose}></button>
          </div>
          <div className="modal-body">
            {assignSuccess && (
              <div className="alert alert-success" role="alert">
                Banner assigned successfully!
              </div>
            )}
            {assignError && (
          ...
          <div className="modal-footer">
            ...
            <button
              ...
              onClick={handleAssignBanner}
              disabled={assignLoading || (!assignToAll && selectedUsers.length === 0)}
            >
              {assignLoading ? "Assigning..." : "Assign Banner"}
            </button>
          </div>
        </div>
      </div>
